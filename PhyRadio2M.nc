...
#define RSSI_OFFSET		-45
   ...
   ***************************************************************************/
	...
	event result_t CC2420SplitControl.initDone()
	{
		...
		atomic def_backoff_us =
		    CC2420_SYMBOL_UNIT * ((call Random.rand() % 0x3F) + 1);
   ...
   **/
	void sendPacket()
	{
		...
		call HPLChipcon.cmd((backoff) ? CC2420_STXONCCA :
				    CC2420_STXON);
		...
		if ((status >> CC2420_TX_ACTIVE) & 0x01) {
			...
			if (!
			    (setBackoffTimer(getCongestionBackoff(txbufptr))))
			{
				sendFailed();
			}
		}
   ...
   */
	task void startSend()
	{
		...
		if (!
		    (call
		     HPLChipconFIFO.writeTXFIFO(txlength + 1,
						(uint8_t *) txbufptr))) {
			sendFailed();
			return;
		}
 ...
 /**********************************************************
   ...
   */
						       ...
						       uint8_t * data)
	{
		...
		//trace(DBG_USR1, "info.strength = %d, %d\r\n", (int8_t)data[length-2], call SignalStrength.getRSSI());
	...
	int16_t getCongestionBackoff(uint8_t * pkt)
	{
		...
		atomic {
			if (flags & FLAG_BACKOFF_RANDOM) {
				time =
				    (call Random.rand() % max_backoff_us) +
				    min_backoff_us;
			} else {
				time = def_backoff_us;
